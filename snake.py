...
def snakeMove(inq, num):
    snakeSpeeds = random.randint(6, 16)
    direction = random.randint(0, 36)
    j1speed = random.randint(50,120)
    j1amp = random.randint(5, 90)
    j2amp = random.randint(2, 5)
    j4amp = random.randint(10, 30) # 130 mid max range 30
    j6amp = random.randint(10, 30)
    curIP = IP[num].copy()
    ...
    while inq.empty() == False:
        ...
        newpos[0] = j1
        arms[num].set_servo_angle_j(angle=initial, is_radian=False)
