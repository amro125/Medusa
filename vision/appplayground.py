...
def strummer(inq, num):
    ...
    while True:
        ...
        if play == 1:  # waving HI
            ...
            print(poseI)
            print(poseF)
            newPos = poseToPose(poseI, poseF, 3)
            gotoPose(num, newPos)
        ...
        if play == 3:  # twirl
            ...
            print(poseI)
            print(poseF)
            newPos = poseToPose(poseI, poseF, 2)
            gotoPose(num, newPos)
            i = 0
            direction = i % 2
...
if __name__ == '__main__':
    ...
    dispatcher.map("/gesture", respondToWave)
    ...
    # SOCKET_CLIENT = '0.0.0.0'
    # SOCKET_PORT = 58327
    #
    # print("connecting socket")
    # s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # s.bind((SOCKET_CLIENT, SOCKET_PORT))
    # s.settimeout(None)
    # s.listen()
    # s.settimeout(None)
    # conn, addr = s.accept()
